...
Run automatic installer script with `sh install.sh` as root or install manually by building with `g++ -std=c++11 cephfssyncd.cpp -o cephfssyncd` and move the build output to /usr/bin/ as cephfssyncd. Place cephfssyncd.service in /etc/systemd/system/. 
